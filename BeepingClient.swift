...
public actor BeepingClient {
    ...
    /// Trace-ID this client correlates logs + `X-Trace-Id` with — either
    /// builder-injected (`.traceID(_:)`) or autogenerated by the wrapper.
    /// Exposed for tests to verify the builder wiring (BEE-2309).
    internal var traceID: String { wrapper.currentTraceID }
    ...
    public static func cloud(apiKey: String, endpoint: URL) -> BeepingClientBuilder {
        BeepingClientBuilder(encoderFactory: { wrapper, mode in
            CloudEncoder(
                ...
                traceID: wrapper.currentTraceID)
        })
    }
